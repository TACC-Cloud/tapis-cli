...
class TestMetaDescriptions(object):
    ...
    def test_meta_update_description(self):
        ...
        assert cmd.get_description().startswith('Update a Metadata document by UUID')

    def test_meta_delete_description(self):
        app = Tapis_App()
        cmd = metadata.MetadataDelete(app, None)
        assert cmd.get_description().startswith('Delete a Metadata document by UUID')

    def test_meta_show_description(self):
        app = Tapis_App()
        cmd = metadata.MetadataShow(app, None)
        assert cmd.get_description().startswith('Show a Metadata document by UUID')

    def test_meta_search_description(self):
        app = Tapis_App()
        cmd = metadata.MetadataSearch(app, None)
        assert cmd.get_description().startswith('Search for Metadata documents')

    def test_meta_pems_list_description(self):
        app = Tapis_App()
        cmd = metadata.MetadataPemsList(app, None)
        assert cmd.get_description().startswith('List Permissions for a Metadata document')

    def test_meta_pems_show_description(self):
        app = Tapis_App()
        cmd = metadata.MetadataPemsShow(app, None)
        assert cmd.get_description().startswith('Show Permissions on a Metadata document for specific User')

    def test_meta_pems_grant_description(self):
        app = Tapis_App()
        cmd = metadata.MetadataPemsGrant(app, None)
        assert cmd.get_description().startswith('Grant Permissions on a Metadata document to a User')

    def test_meta_pems_revoke_description(self):
        app = Tapis_App()
        cmd = metadata.MetadataPemsRevoke(app, None)
        assert cmd.get_description().startswith('Revoke Permissions on a Metadata document for a User')

    def test_meta_pems_drop_description(self):
        app = Tapis_App()
        cmd = metadata.MetadataPemsDrop(app, None)
        assert cmd.get_description().startswith('Drop all granted Permissions from a Metadata document')
