...
def grant(app_id, username, permission, grant_system_roles=True, permissive=False, agave=None,
          **kwargs):
    ...
    try:
        ...
        if permission.upper() in ['ALL', 'READ_EXECUTE', 'WRITE_EXECUTE', 'EXECUTE']:
            if grant_system_roles:
                app_def = agave.apps.get(appId=app_id)
                systems = [app_def['executionSystem'], app_def['deploymentSystem']]
                for s in systems:
